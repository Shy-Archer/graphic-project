...
void bud3::loadObj(std::string plik) {
...
void bud3::setup(const char* filenamepng, string filenameobj) {
...
void bud3::draw(ShaderProgram* sp, GLint textureUnits)

{
	M1 = glm::mat4(1.0f);
	M1 = glm::translate(M1, glm::vec3(-45.0f, -9.7f, -15.0f));
	M1 = glm::translate(M1, glm::vec3(x_coord, 0.0f, 0.0f));
	M1 = glm::translate(M1, glm::vec3(0.0f, 0.0f, 0.0f));
	M1 = glm::rotate(M1, 90.0f*3.14f /180.0f, glm::vec3(-1.0f, 0.0f, 0.0f)); //Compute model matrix
	M1 = glm::scale(M1, glm::vec3(-0.5f, 0.25f, 0.10f));
