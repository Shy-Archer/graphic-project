...
void prawanoga::loadObj(std::string plik) {
...
void prawanoga::setup(const char* filenamepng, string filenameobj) {

	TEX = readTexture(filenamepng);
	loadObj(string(filenameobj));
}
void prawanoga::draw(ShaderProgram* sp, GLint textureUnits, glm::mat4 Mp) {
	M1 = Mp;
	//M1 = glm::rotate(Mp, 90.0f * 3.14F / 180.0f, glm::vec3(0.0f, 1.0f, 0.0f)); //Compute model matrix

	M1 = glm::translate(M1, glm::vec3(-0.69f, 0.11f, 0.1f));
